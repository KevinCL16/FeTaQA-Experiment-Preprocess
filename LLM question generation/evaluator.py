...
def postprocess_text(preds, labels, metric_name):
    ...
    for pred in preds:
        if "Answer:" in pred:
            idx_a.append(pred.index("Answer:") + 8)
        else:
            idx_a.append(0)
    preds_2 = []
    for pred, i in zip(preds, range(len(preds))):
        preds_2.append(pred[idx_a[i]:])
    ...
    if metric_name == "rouge":
        ...
        preds_2 = [pred.split(' ') for pred in preds]
        labels = [[label.split(' ')] for label in labels]
    else:
        pass

    return preds_2, labels
...
if __name__ == '__main__':
    ...
    with open("2003_TEST_1-shot_by_gpt-3.5_with_specific_style_requirement.jsonl", encoding='utf-8') as f:
